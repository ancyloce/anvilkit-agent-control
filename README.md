...
| `internal/adapters/modelproxy` | The original-identity query of model dispatches against the Model Proxy's frozen transport (P11); business writes stay with the development double until the Pagix declaration (ENV-07) |
...
The service reads one reviewed, secret-free file (`config.yaml`, path from `ANVILKIT_CONTROL_CONFIG`, default `./config.yaml`) with the sections `grpc` (listen, capacities, shutdown), `inventory` (backend `filesystem` or `s3`), `artifacts` (backend `disabled` or `s3`, bounds), `temporal`, `relay`, `profiles`, `recovery`, `dispatch` (with its file-only DEVELOPMENT_ONLY fixtures) and `model_proxy` (the Model Proxy placement of the original-identity query of model dispatches, P11, `internal/adapters/modelproxy`: with an address recovery asks the Proxy's `GET /api/v1/model-calls/{callId}` about an unresolved model dispatch of the configured owner and records the answer as an observation under the source `query:model-proxy`; without one the DEVELOPMENT_ONLY attestation double answers). The only environment overrides are `ANVILKIT_CONTROL_LISTEN`, `ANVILKIT_CONTROL_TEMPORAL_ADDRESS`, `ANVILKIT_CONTROL_INVENTORY_DIR`, `ANVILKIT_CONTROL_INVENTORY_S3_{ENDPOINT,BUCKET,ACCESS_KEY_ID,SECRET_ACCESS_KEY}`, `ANVILKIT_CONTROL_ARTIFACTS_S3_{ENDPOINT,BUCKET,ACCESS_KEY_ID,SECRET_ACCESS_KEY}`, `ANVILKIT_CONTROL_MODEL_PROXY_ADDRESS`, the secret `ANVILKIT_CONTROL_MODEL_PROXY_TOKEN` and the secret `ANVILKIT_CONTROL_DATABASE_URL`; any other `ANVILKIT_CONTROL_*` variable stops the process, and a secret found in the file refuses the candidate. The gRPC health service reports `SERVING` once the listener is up and the relay runs, `NOT_SERVING` while the server drains.
...
The contract module is an ordinary published dependency. This build requires `github.com/ancyloce/anvilkit-agent-contracts/go v0.1.2`: the tag `go/v0.1.2` (commit `532f680` on `main` of the `anvilkit-agent-contracts` repository, tagged and pushed 2026-09-16; it carries `ExecutionService.GetInstance` with the attempt's `operation` in its answer, `profile.expectedResult.resultSizeBytes` and the `npm`, `browser` and `css` artifact classes), served by `proxy.golang.org` (origin `refs/tags/go/v0.1.2`) and verified against the checksum database (`go.sum`: `h1:BgUX79+lfJxtUlpzgNbyzejMgYvFO8iqORBDqBAvCgo=`, go.mod `h1:GpREiampo5pSy0+IsLQcbA+DVi6jwpweI3ojWuv/gs4=`; the zip includes that repository's root `LICENSE`, as Go requires for a module in a subdirectory); before the tag existed the same line named the pseudo-version of `a397fee`, and nothing but the `require` line and the two `go.sum` lines changed with the tag. No replace directive, workspace or local proxy is involved.
